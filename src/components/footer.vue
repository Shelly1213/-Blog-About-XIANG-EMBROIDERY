...
<template>
    ...
    <div v-if="this.$store.state.themeObj.user_start!=0" class="footBack footBackQian">
        <div class="footer-img" :style="{background:this.$store.state.themeObj.bottom_img?'url('+this.$store.state.themeObj.bottom_img+') no-repeat 50%':'url(static/img/foot02.png) no-repeat 50%'}">
        </div>
        <div class="fcontainer">
            <p>
              {{$t("footer.1")}}  <span v-html='longTime'>{{longTime}}</span><span class="timeJump"></span>
            </p>
            <p>
              <a href="https://www.aliyun.com/?spm=a2c49.11131515.0.0.5Z9AkR" target="_blank">{{$t("footer.2")}}</a>{{$t("footer.3")}}
            </p>
            <p v-if="this.$i18n.locale ==='zh'">
                © 2020 <a href="https://www.hnust.edu.cn">{{$t("footer.4")}}</a>.  由<a href="https://cn.vuejs.org/" target="_blank">Vue</a> 强力驱动.
            </p>
            <p v-else>
              © 2020 <a href="https://www.hnust.edu.cn">{{$t("footer.4")}}</a>. Powered by<a href="https://cn.vuejs.org/" target="_blank">Vue</a>
            </p>
            <p>
                {{$t("footer.5")}}
            </p>
...
<script>
    export default {
        data() { //选项 / 数据
            return {
                footBg:"url(static/img/footer02.png) no-repeat 50%",//底部图片
                longTime:'',
            }
        },
        methods: { //事件处理器
            runTime:function(){//运行倒计时
                var that = this;
                var oldTime =new Date('2020/11/30 00:00:00');
                var timer = setInterval(function(){
                  ...
                  if(that.$i18n.locale==='zh')
                    ...
                    that.longTime = days+"d " + hours+"h " + minutes+"m "+seconds+"s";
                },1000)
            }
        },
        components: { //定义组件
