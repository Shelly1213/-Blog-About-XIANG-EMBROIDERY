			...
			<el-col :span="24">
				...
				<div class="headBox">
					<el-menu :default-active="activeIndex" class="el-menu-demo" mode="horizontal" @select="handleSelect" :router="true">
						<el-menu-item index="/Home"><i class="fa fa-wa fa-home"></i>{{$t("header.2")}}</el-menu-item>
            <el-submenu index="/Aboutme">
              <template slot="title"><i class="	fa fa-hand-o-down"></i> {{$t("header.3")}}</template>
<!--              <el-menu-item v-for="(item,index) in projectList" :key="'class2'+index" :index="'/Share2' + item.nav_url">{{item.nav_name}}</el-menu-item>-->
              <el-menu-item  index="/Share21">{{$t("header.8")}}</el-menu-item>
              <el-menu-item  index="/Share22">{{$t("header.9")}}</el-menu-item>

            </el-submenu>
            <el-menu-item index="/Message"><i class="fa fa-wa fa-pencil"></i> {{$t("header.4")}}</el-menu-item>
						<el-submenu index="/Share">
							<template slot="title"><i class="fa fa-wa fa-archive"></i> {{$t("header.5")}}</template>
<!--							<el-menu-item v-for="(item,index) in classListObj" :key="'class1'+index" :index="'/Share1'+item.class_id">{{item.cate_name}}</el-menu-item>-->
              <el-menu-item  index="/Share11">{{$t("header.10")}}</el-menu-item>
              <el-menu-item  index="/Share12">{{$t("header.11")}}</el-menu-item>
            </el-submenu>

						<el-menu-item index="/Reward"><i class="fa fa-wa fa-cny"></i> {{$t("header.6")}}</el-menu-item>
						<el-menu-item index="/Friendslink"><i class="fa fa-wa fa-users"></i> XXX</el-menu-item>
						<el-menu-item index="/Aboutus"><i class="fa fa-wa fa-vcard"></i> {{$t("header.7")}}</el-menu-item>

            <div class="userInfo">
                <el-submenu index="/changeLanguage">
                  <template slot="title">{{$t("common.changeLanguage")}}</template>
                  <el-menu-item v-for="(item,index) in langlistobject" :key="'class1'+index" @click="changeLanguage(item.cate_name)">{{item.cate_name}}</el-menu-item>
                </el-submenu>
            </div>
            ...
            <el-collapse-transition>
              <el-menu :default-active="activeIndex" class="mlistmenu" v-show="!pMenu" theme="dark" @open="handleOpen" @close="handleClose" :unique-opened="true" :router="true">
                ...
                <el-submenu index="2">
                  <template slot="title"><i class="fa fa-hand-o-down"></i> {{$t("header.3")}}</template>
                  <el-menu-item  index="/Share21">{{$t("header.8")}}</el-menu-item>
                  <el-menu-item  index="/Share22">{{$t("header.9")}}</el-menu-item></el-submenu>
                <el-menu-item index="/Message"><i class="fa fa-wa fa-pencil"></i> {{$t("header.4")}}</el-menu-item>
                <el-submenu index="/Share">
                  <template slot="title"><i class="fa fa-wa fa-archive"></i> {{$t("header.5")}}</template>
                  <el-menu-item  index="/Share11">{{$t("header.10")}}</el-menu-item>
	...
	<div class="headImgBox" :style="{backgroundImage:this.$store.state.themeObj.top_image?'url('+this.$store.state.themeObj.top_image+')':'url(static/img/background.jpg)'}">
		<div class="scene">
		...
		<div class="h-information">
			...
			<h2 class="h-description">
                    <a href="#/Aboutus">
                        {{this.$store.state.themeObj.autograph?this.$store.state.themeObj.autograph:$t("header.1")}}
                    </a>
                </h2>
		</div>
	</div>
...
export default {
	data() { //选项 / 数据
		return {
			...
			headTou: '', //头像
			projectList: '' ,//项目列表
      langlistobject: [{cate_name:"zh"},{cate_name:"en"}]
		}
	},
	watch: {
	...
	created() { //生命周期函数
		...
		var onVisibilityChange = function() {
			if (document[hiddenProperty]) { //被隐藏
				document.title = '等你传送回~';
			} else {
				document.title = '再次降落~'; //当前窗口打开
